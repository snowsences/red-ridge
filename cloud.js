...
export async function connect(config,trusted,callbacks){
 ...
 return {signIn:()=>signInWithPopup(auth,new GoogleAuthProvider()),signOut:async()=>{stop();await signOut(auth);await terminate(db);if(trusted)await clearIndexedDbPersistence(db)},wait:()=>waitForPendingWrites(db),write(ops){if(auth.currentUser?.uid!==OWNER)throw Error('Sign in to save.');const b=writeBatch(db);for(const op of ops){const ref=doc(col(op.collection||'records'),op.id);if(op.remove)b.delete(ref);else b.set(ref,op.value)}return b.commit()}};
}
